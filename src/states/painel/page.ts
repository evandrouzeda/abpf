...
export default class RestaurantePage extends Page {
    ...
    async create(obj?: any): Promise<Zeyo> {
        ...
        return this.main = Z("main").attribute("style", "background-color: #f0f0f0; height: 100vh; height: 100svh; display: grid; grid-template-rows: 12% 88%;").children(
            ...
            Z("section").class("p-m").children(
                Z("div").children(
                    Z("div").class("d-flex", "jc-between").children(
                        Z("h2").text("Restaurantes"),
                        Z("button").text("Adicionar")
                    )
                ).object(async o => {
                    const body = Z("div").children(new Load(this.app).create())
                    o.children(body);
                    const result = await new Promise<string>(res => setTimeout(() => {
                        res("teste")
                    }, 2000));
                    body.HTML("");
                    body.children(Z("p").text(result));
                }),
            )
        )
    }
